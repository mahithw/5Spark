...
export function UpcomingTasks({
  ...
  const [showAddTask, setShowAddTask] = useState(false);
    ...
    try {
      const res = await fetch("http://localhost:4000/api/google/auth-url");
  ...
  async function handleImportTodayTasks() {
    try {
      const res = await fetch("http://localhost:4000/api/calendar/tasks-today");
      if (!res.ok) {
        if (res.status === 401) {
      ...
      const data = await res.json();
      const calendarTasks = data.tasks as Task[];

      // Use the existing onAddTask callback to add them into app state
      calendarTasks.forEach((task) => {
        onAddTask(task);
      });

      alert(`Imported ${calendarTasks.length} event(s) from Google Calendar.`);
    } catch (err) {
      console.error("Failed to import calendar tasks", err);
      alert("Could not import tasks from Google Calendar.");
    }
  }
        ...
        >
          <div className="p-4 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-white flex-shrink-0">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              type="button"
              onClick={(e) => {
                e.stopPropagation(); // don’t open the big calendar dialog
                handleConnectGoogleCalendar();
              }}
              className="px-2 py-1 text-xs rounded bg-white border border-blue-300 text-blue-600 hover:bg-blue-50"
            >
